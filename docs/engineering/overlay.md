...
Overlay transcript 使用 Pi TUI 的 `Container`、`Box`、`Text` 和 `Spacer` 组合渲染。用户消息、Context、Pull、普通工具和 Advice Block 的全宽背景由 `Box` 负责，颜色直接使用 Pi theme token；Advice 使用现有错误背景作为视觉别名，标题和正文使用 `text`。Pull 默认展示前 5 个结构化 display item；Overlay 通过注入的 KeybindingsManager 监听 `app.tools.expand`，统一切换所有 Pull Block 的展开状态，并在关闭、重新打开期间保留该状态。用户解绑该 action 时，折叠提示只报告剩余条目数，不展示不可用的快捷键。
...
Pull Block 使用 `pull_transcript` tool result details 中的 display items，不解析返回给 Advisor 模型的 markdown。Display item 与 markdown 由同一次 Primary Transcript View slice 序列化产生，tool call 与 tool result 合并为一个 item，因此 header 的 `N msgs` 表示 Overlay 最终展示的逻辑条目数。
