...
function primaryArg(name: string, args: Record<string, unknown> | undefined): string {
	...
	// Advisor advice is the most informative summary; preserve kind too.
	if (name === "advise") {
		const advice = typeof args.advice === "string" ? args.advice : "";
		const kind = typeof args.kind === "string" ? args.kind : "";
		if (advice && kind) return oneLine(`${kind}: ${advice}`);
		if (advice) return oneLine(advice);
		if (kind) return oneLine(kind);
		return "{}";
	}
	if (name === "grep") {
